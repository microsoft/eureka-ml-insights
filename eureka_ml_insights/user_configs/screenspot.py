...
class SCREENSPOT_NORMALIZED_PIPELINE(ExperimentConfig):
    ...
    def configure_pipeline(self, model_config: ModelConfig, resume_from: str = None, **kwargs: dict[str, Any] ) -> PipelineConfig:
        ...
        self.evalreporting_comp = EvalReportingConfig(
            ...
            metric_config=MetricConfig(BboxMetric, {"normalize_mode": BboxMetric.NormalizeMode.NORMALIZED, "xywh": True}),
            aggregator_configs=[
...
class SCREENSPOT_NORMALIZED_1000_PIPELINE(SCREENSPOT_NORMALIZED_PIPELINE):
    """
...
class SCREENSPOTV2_NORMALIZED_PIPELINE(SCREENSPOT_NORMALIZED_PIPELINE):
    ...
    def configure_pipeline(self, model_config: ModelConfig, resume_from: str = None, **kwargs: dict[str, Any] ) -> PipelineConfig:
        ...
        self.data_processing_comp.data_reader_config.init_args["path"] = "/data/datasets/EVAL/ScreenSpot/screenspot_all_v2.jsonl"
        self.inference_comp.data_loader_config.init_args["mm_data_path_prefix"] = "/data/datasets/EVAL/ScreenSpot/images_v2"
        return config
    
class SCREENSPOTV2_NORMALIZED_1000_PIPELINE(SCREENSPOT_NORMALIZED_1000_PIPELINE):
    """
...
class SCREENSPOT_PRO_NORMALIZED_1000_PIPELINE(SCREENSPOT_NORMALIZED_1000_PIPELINE):
    """
    This defines an ExperimentConfig pipeline for the SCREENSPOT_PRO dataset using normalized coordinates.
    There is no model_config by default and the model config must be passed in via command line.
    """
